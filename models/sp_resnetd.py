...
class Model(nn.Module):
    ...
    def __init__(self, num_classes=1000, input_size=224):
        ...
        head_layers = [
            conv_m := SConv2d(
                in_channels_list=[3 for _ in range(len(channels))], out_channels_list=channels,
        ...
        cfg = [channels[-1]]
        for i, out_ch in enumerate([32, 64], start=1):
            out_channels = [int(out_ch * width_mult) for width_mult in FLAGS.width_mult_list]
            cfg.append(out_channels[-1])
            head_layers += [
                conv_m := SConv2d(
                    in_channels_list=channels, out_channels_list=out_channels,
                    kernel_size=3, stride=1, padding=1, bias=False),
                bn_m := norm_layer(out_channels),
                nn.ReLU(inplace=True),
            ]
            ...
            prev_conv_m = conv_m
            prev_bn_m = bn_m
        head_layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        cfg.append('M')

        self.features.append(nn.Sequential(*head_layers))
